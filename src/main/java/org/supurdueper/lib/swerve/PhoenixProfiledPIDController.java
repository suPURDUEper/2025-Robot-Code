 ...
 *
 * <p>Implements a PID control loop whose setpoint is constrained by a trapezoid profile. Users should call reset() when
 * they first start running the controller to avoid unwanted behavior.
 *
 * <p>This class differs from the WPI implementation by using explicit timestamps for integral/derivative calculations.
 * Ideally, these timestamps come from the {@link com.ctre.phoenix6.StatusSignal}.
 */
public class PhoenixProfiledPIDController implements Sendable {
    ...
    /**
     * Sets the IZone range. When the absolute value of the position error is greater than IZone, the total accumulated
     * error will reset to zero, disabling integral gain until the absolute value of the position error is less than
     * IZone. This is used to prevent integral windup. Must be non-negative. Passing a value of zero will effectively
     * disable integral gain. Passing a value of {@link Double#POSITIVE_INFINITY} disables IZone functionality.
     *
     ...
     *
     * <p>The internal integrator is clamped so that the integral term's contribution to the output stays between
     * minimumIntegral and maximumIntegral. This prevents integral windup.
     *
     ...
     * @return The controller's next output.
     */
    public double calculate(double measurement, double goal, double currentTimestamp) {
        setGoal(goal);
        return calculate(measurement, currentTimestamp);
    }
    ...
    public void initSendable(SendableBuilder builder) {
        builder.setSmartDashboardType("PhoenixProfiledPIDController");
